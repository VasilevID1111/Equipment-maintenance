...
namespace Equipment_maintenance
{
    public partial class Form4 : Form //акт приема передачи
    {
        ...
        private void insertBD(List<string> act)
        {
            ...
            try
            {
                NpgsqlCommand com = new NpgsqlCommand($"insert into handover_acts values ({act[0]}, '{act[1]}', '{act[2]}', '{act[3]}', '{act[4]}', '{act[5]}', '{act[6]}', {act[7]})", conn); //insert-им акт передачи
                ...
                NpgsqlCommand com4 = new NpgsqlCommand($"insert into equipment_movements values ({idDvig}, '{act[3]}', '{act[4]}', '{act[8]}', 'Акт приема-передачи', {act[0]}, '{act[2]}')", conn); //insert-им движение
                com4.ExecuteNonQueryAsync(); //выполняем запрос

                //и update оборудования
                NpgsqlCommand com5 = new NpgsqlCommand($"update equipment set idАдреса = {act[8]}, Статус = 1 where idОборудования = '{act[3]}'", conn); //insert-им движение
        ...
        private string idDoc() //берем id для нового документа (прибавляем 1 к старому)
        {
            ...
            NpgsqlCommand com = new NpgsqlCommand($"SELECT idДокумента FROM handover_acts ORDER BY idДокумента DESC LIMIT 1", conn); 
            DataTable dt = new DataTable();
        ...
        private void addressBD()
        {
            ...
            {
                //TextBox[] tbs = { textBox1, textBox2, textBox3, textBox4, textBox5, textBox6, textBox7 };
                Label[] lbls = { label2, label3, label5, label6, label7, label8 };
